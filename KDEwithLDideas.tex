...
%Luke Bielawski (8:25pm 6/19): I think this expression can be taken from https://arxiv.org/pdf/math/0601149 to be roughyl something like: sum over all of the partitions of \mathfrak{u} and then take the product over the elements of that partition. I am thinking how to keep it in the current notation without thinking about partitions
...
\section{Evaluating the error of the bias term}
To understand the dependence of \eqref{eq:bias_error} bandwidth, $h$, let's approximate $\tKY$ by a MacLaurin series with $r+2$ terms plus an error term.  Since $\tKY$ is an even function, the odd numbered derivatives drop out.
\begin{equation*}
	\tKY(y)  = \sum_{\ell = 0}^{r+1} \frac{\tKY^{(2\ell)}(0) \, y^{2\ell}}{2\ell!} + \frac{\tKY^{(2r+4)}(\eta) \,  y^{2r+4}}{(2r+4)!},
\end{equation*}
where $\eta$ is between $0$ and $y$.
Here $\tKY^{(2\ell)}$ denotes the $2\ell^{\text{th}}$ derivative of $\tKY$.

It follows from this series expression for $\tKY$ that
\begin{align} \label{eq:isomidterm}
	\int_{-\infty}^{\infty} \tk(w) \, \tKY(hw) \, \dif w & =
	\int_{-\infty}^{\infty} \tk(w) \, \sum_{\ell = 0}^{r+1} \frac{\tKY^{(2\ell)}(0) \, (hw)^{2\ell}}{2\ell!}  \, \dif w 
	+ \int_{-\infty}^{\infty} \tk(w) \, \frac{\tKY^{(2r+4)}(\eta) \,  (hw)^{2r+4}}{(2r+4)!} \, \dif w \\
 \nonumber & =
	\sum_{\ell = 0}^{r+1} \frac{\tKY^{(2\ell)}(0) h^{2\ell}}{2\ell!}\int_{-\infty}^{\infty} \tk(w) \, w^{2\ell}   \, \dif w 
	+ \frac{h^{2r+4}}{(2r+4)!} \int_{-\infty}^{\infty} \tk(w) \, \tKY^{(2r+4)}(\eta) \,  w^{2r+4} \, \dif w ,
\end{align}
since $\tk$ and $\tKY$ are even.  If $\tk$ has the property that
\begin{equation} \label{eq:tkprop}
	\int_{-\infty}^{\infty} \tk(w) w^{2\ell}  \, \dif w =  \delta_{0,\ell}, \qquad l=0,\dots,r,
\end{equation}
then 
\begin{multline} \label{eq:isomidterm_b}
	\int_{-\infty}^{\infty} \tk(w) \, \tKY(hw) \, \dif w  = \tKY(0) + 
 \frac{\tKY^{(2r+2)}(0) h^{2r+2}}{(2r+2)!}\int_{-\infty}^{\infty} \tk(w) \, w^{2r+2}   \, \dif w  \\
	+ \frac{h^{2r+4}}{(2r+4)!} \int_{-\infty}^{\infty} \tk(w) \, \tKY^{(2r+4)}(\eta) \,  w^{2r+4} \, \dif w.
\end{multline}

We next look at 

\textbf{Attempted Solution (Luke Bielawski with modifications by Fred)}
\begin{align*}
\MoveEqLeft{\int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tKY(h(w-v)) \tk(v) \, \dif w \, \dif v} 
 \\
 \nonumber
 & = \sum_{\ell = 0}^{r+1} \frac{\tKY^{(2\ell)}(0) h^{2\ell}}{2\ell!}
 \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) (w-v)^{2\ell}  \, \dif w \, \dif v \\
 \nonumber
&\qquad \qquad  + 
 \frac{h^{2r+4}}{(2r+4)!} \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) \, \tKY(\eta)(v-w)^{2r+4} \dif w \, \dif v
\end{align*}
Again under assumption  \eqref{eq:tkprop} all integrals of the form 
$ \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) w^p v^q  \, \dif w \, \dif v$ with $p, q \in \{0, \ldots, 2r+1\}$ and $p >0$ or $q>0$ vanish.  This leaves us with 
\begin{align}
\MoveEqLeft{\int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tKY(h(w-v)) \tk(v) \, \dif w \, \dif v} 
 \\
 \nonumber
 & = \tKY(0) + \frac{\tKY^{(2r+2)}(0) h^{2r+2}}{(2r+2)!}
 \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) (w^{2r+2} + v^{2r+2})  \, \dif w \, \dif v \\
 \nonumber
&\qquad \qquad  + 
 \frac{h^{2r+4}}{(2r+4)!} \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) \, \tKY(\eta)(v-w)^{2r+4} \dif w \, \dif v \\
  \nonumber
 & = \tKY(0) + \frac{2\tKY^{(2r+2)}(0) h^{2r+2}}{(2r+2)!}
 \int_{-\infty}^\infty \tk(w) w^{2r+2}   \, \dif w  \\
 \nonumber
&\qquad \qquad  + 
 \frac{h^{2r+4}}{(2r+4)!} \int_{-\infty}^\infty \int_{-\infty}^\infty \tk(w) \tk(v) \, \tKY(\eta)(v-w)^{2r+4} \dif w \, \dif v
\end{align}
