...
\end{equation*}
where $\{\gamma_j\}_{j=1}^d$ are coordinate weights \cite{Hic97a,Hic98b}.
\begin{align*}
...
\begin{equation*}
    \frac{\var(\tk((y - f(\cdot))/h)/h)}{n} =  \frac{\var(\tk((y - f(\cdot))/h))}{h^2 n},
\end{equation*}
...
\section{The Reproducing Kernel Hilbert Space Trick}
