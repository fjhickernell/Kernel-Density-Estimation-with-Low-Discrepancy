...
Let's back up.  Any function $g:[0,1]^d \to \reals$ can be decomposed into pieces only depending on coordinates indexed by the elements of the set $\fu \subseteq \{1, \ldots, d\}$.  This decomposition is defined recursively:
\begin{align*}
    g_\emptyset &= g(1/2, \ldots, 1/2), \\
...
Now what does this decomposition look like when $g(\vx) = \tk\bigl((y-f(\vx))/h\bigr)/h$?
\begin{align*}
...
Moreover, decompose the reproducing kernel as follows:
\begin{align*}
    K_{\vx,\fu} (\vt_\fu,\vx_\fu) &:= \frac{1}{2^{\abs{\fu}}}\prod_{j \in \fu} [\abs{t_j -1/2} + \abs{x_j-1/2} - \abs{t_j  - x_j}] \\
    \KX(\vx,\vt) & = \sum_{\fu \subseteq \{1, \ldots, d\}} \gamma_\fu^2   K_{\vx,\fu} (\vt_\fu,\vx_\fu).
\end{align*}
Then, recalling $g(\vx) = \tk\bigl((y-f(\vx))/h\bigr)/h$, it follows that
\begin{align*}
	%\label{eq:secondbddecomp}
\abs{\int_{[0,1]^d} k(f(\vx),y) \,  \dif (F_{\mathrm{unif}} - F_{\mX})(\vx)}^2
...
& = \abs{\sum_{\fu \subseteq \{1, \ldots, d} \ip[K_{\vx,\fu}]{\int_{[0,1]^{\fu}} K_{\vx,\fu}(\cdot,\vx_\fu) \,  \dif (F_{\mathrm{unif}} - F_{\mX})(\vx)}{g_u}}^2 \\
\nonumber
& \le  \sum_{\fu \subseteq \{1, \ldots, d\}} \norm[K_{\vx,\fu}]{\int_{[0,1]^\fu} K_{\vx,\fu}(\cdot,\vx) \,  \dif (F_{\mathrm{unif}} - F_{\mX})(\vx)}^2\, \norm[K_{\vx,\vu}]{g_\fu } ,
\end{align*}
