...
Defined the random variable $Y = f(\vX)$, where $\vX \sim \cu[0,1]^d$.  Low discrepancy sequences are used for approximating  $\mu = \Ex(Y)$.  Can they be used for estimation of $\varrho$, the probability density (or mass) function of $Y$?

Let $\nu$ be a probability mass or density function.  A generalized kernel density estimator (KDE), $\hvarrho(\cdot,\nu,k)$, can be defined by the following Riemann-Stieltjes integral\footnote{Standard notation would express this Riemann-Stieltjes integral as $\int_{-\infty}^{\infty} k(z,y) \, \dif N(z)$, where $\nu(z) = N'(z)$ in a generalized sense.  However, we opt for a more intuitive notation since we are working with probability density/mass functions.}:
\begin{equation}
	\label{eq:genkde}
...
\end{equation}
This ensures that $\displaystyle \int_{-\infty}^{\infty} \hvarrho(y,\nu,k) \, \dif y = 1$.  We do not require $k$ to be non-negative, which means that $\hvarrho(y,\nu,k)$ might be negative for some values of $y$.  For \eqref{eq:genkde} to be a good estimator, the density $\nu$ should approximate $\varrho$ well.

Let $\varrho_{\vy}$ be the empirical probability mass function of a vector of sampled $Y$ values, $\vy = (y_1, \ldots, y_n)$.  Then a \emph{practical KDE} is
\begin{equation*}
